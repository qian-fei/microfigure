...
def get_data_statistics():
    ...
    try:
        ...
        pipeline = [
            {"$match": {"$and": [{"$gte": ["$date", last_stamp]}, {"$lte": ["$date", today_stamp]}]}},
        ...
        pipeline = [
            {"$match": {"$and": [{"$gte": ["$date", last_stamp]}, {"$lte": ["$date", today_stamp]}]}},
            {"$group": {"_id": None, "total_amount": {"$sum": "$amount"}, "total_sale": {"$sum": "$sale"}}},
            {"$group": {"_id": None, "total_amount": 1, "total_sale": 1}}
        ]
        cursor = manage.client["user_statistical"].aggregate(pipeline)
        data_list = [doc for doc in cursor]
        if not data_list:
        ...
        pipeline = [
            {"$match": {"$and": [{"$gte": ["$date", last_befor_stamp]}, {"$lte": ["$date", last_stamp]}]}},
            {"$group": {"_id": None, "total_amount": {"$sum": "$amount"}, "total_sale": {"$sum": "$sale"}}},
            {"$group": {"_id": None, "total_amount": 1, "total_sale": 1}}
        ]
        cursor = manage.client["user_statistical"].aggregate(pipeline)
        ...
        amount_delta = (today_total.get("total_amount") - befor_total.get("total_amount")) / befor_total.get("total_amount")
        sale_delta = (today_total.get("total_sale") - befor_total.get("total_sale")) / befor_total.get("total_amount")
        amount_delta = "%.2f" % amount_delta
        sale_delta = "%.2f" % sale_delta
        today_total["amount_delta"] = amount_delta
        today_total["sale_delta"] = sale_delta
