...
def get_banner(banner_max=3):
    ...
    try:
        pipeline =[
            ...
            {"$sort": SON([("order", 1)])},
            {"$limit": banner_max},
            {"$project": {"_id": 0, "pic_url": {"$concat": ["http://192.168.1.183:8080", "$pic_url"]}, "order": 1, "link": 1}}
        ]
...
def get_label_kw(kw_max=5, label_max=5):
    ...
    try:
        ...
        if not doc:
            ...
            doc = manage.client["custom_label"].find_one({"user_id": user_id, "type": type, "state": 1}, {"_id": 0}).limit(label_max)
            label = doc.get("label")
        data["label"] = label
...
def get_pic_detail(domain=constant.DOMAIN):
    ...
    try:
        ...
        # 格式价格信息
        cursor = manage.client["price"].find({"type": 0}, {"_id": 0})
        price_data = [doc for doc in cursor]
        if not price_data: 
            raise Exception("Price data does not exist")
        ...
        # for i in cursor:
        #     for j in i["pic_item"]:
        #         temp = manage.client["works"].find_one({"pic_id": i.get("uid")})
        #         if temp: 
        #             j["label"] = temp["label"]
        #         j["big_pic_url"] = domain + j["big_pic_url"]
        #         j["thumb_url"] = domain + j["thumb_url"]
        #         altas_data.append(j)
...
def get_hot_keyword(kw_max=10):
    ...
    try:
        ...
        yesterday_timestamp = int(time.mktime(yesterday.timetuple())) * 1000
        pipeline = [
            {"$match": {"$and":[{"create_time": {"$gte": yesterday_timestamp}}, {"create_time": {"$lte": today_timestamp}}], "state": {"$gte": 0}}},
            {"$group": {"_id": "$keyword", "count": {"$sum": 1}}},
            {"$project": {"keyword": "_id.keyword", "count": 1}},
            {"$sort": SON([("count", -1)])},
            {"$limit": kw_max}
        ]
        cursor = manage.client["user_search"].aggregate(pipeline)
        if not cursor : raise Exception("keyword data does not exist")
        data_list_1 = [doc["keyword"] for doc in cursor]
        cursor = manage.client["user_search"].find({"state": 0})
        data_list_2 = [doc["keyword"] for doc in cursor]
