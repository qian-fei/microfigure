...
def post_register(nick_limit=8):
    ...
    try:
        ...
        # 条件
        condition = {
            "uid": str(uid), "nick": str(mobile)[:nick_limit], "sex": "保密", "age": 20, "mobile": str(mobile), "password": password_b64, "head_img_url": "", "state": 1, "account": mobile,
            "type": "user", "balance": 0, "works_num":0, "create_time": int(time.time() * 1000), "update_time": int(time.time() * 1000), "login_time": int(time.time() * 1000)
        }
        # 正常注册
        if not oauth:
        ...
        manage.client["user_statistical"].update({"user_id": uid, "date": today_stamp}, {"$inc": {"register_num": 1}})
        # 生成token
        data = {"uid": str(uid)}
        md5_token = hashlib.md5(str(data).encode()).hexdigest()
        data = {"md5_token": md5_token, "timestamp": int(time.time() * 1000)}
